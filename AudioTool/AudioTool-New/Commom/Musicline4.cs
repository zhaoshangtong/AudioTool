...
namespace AudioToolNew.Commom
{
    ...
    public class Musicline4
    {
        ...
        private List<MusicContent> musicContents = new List<MusicContent>();
        ...
        private void GetBaiduSpeech(object para)
        {
            var data = para as BaiduRead;
            if (data != null)
            {
                double voice_time = data.voice_time;
                TimeSpan startMilliSecond = data.start;
                TimeSpan endMilliSecond = data.end;
                string sound_path = data.sound_path;
                int i = data.file_name;
                string language = data.language;
                double splitTime = data.splitTime;
                string newFile = "";
                string _newFile = "";
                string pcm_newFile = "";
                ...
                var musicContent = musicContents.Where(o => o.voiceMilliSecond == voice_time).FirstOrDefault();
                if (musicContent == null)
                {
                    LogHelper.Error("musicContent为空");
                    return;
                }
                try
                {
                    ...
                    if (Util.isNotNull(server_path))
                    {
                        ...
                        musicContent.voiceFile = server_path;
                    }
                    else
                    {
                        need_delete = false;
                        ...
                        musicContent.voiceFile = Util.getServerPath() + "/NewSoundFiles/" + Path.GetFileNameWithoutExtension(sound_path) + "/" + fileName;
                    }
                    //大于60s的需要再处理
                    if (span == TimeSpan.FromSeconds(50))//音频大于60s,只截取50s
                    {
                        ...
                        musicContent.baidu_text = Rays.Utility.BadiAI.BaiduAI.BaiduTranslateToText(_newFile, language, trimed.WaveFormat.SampleRate.ToString());
                    }
                    else
                    {
                        //将wav转换成pcm可以提高识别速度
                        ...
                        musicContent.baidu_text = Rays.Utility.BadiAI.BaiduAI.BaiduTranslateToText(pcm_newFile, language, trimed.WaveFormat.SampleRate.ToString());
                    }
                }
                catch (Exception ex)
                {
                    LogHelper.Error("GetBaiduSpeechAsync:" + ex.Message);
                }
                finally
                {
                    reader.Close();
            ...
            {
                LogHelper.Error("百度转语言时参数错误，para为空");
            }
        }
        ...
        /// <param name="language"></param>
        /// <returns></returns>
        private List<TextContrastResult> GetTextContrast(List<MusicContent> musicContents, string originalText, string language)
        {
            string[] fileUrls = musicContents.Select(o => o.voiceFile).ToArray();
            double[] timeSpans = musicContents.Select(o => o.voiceMilliSecond).ToArray();
            string[] baidu = musicContents.Select(o => o.baidu_text).ToArray();
            return GetTextContrast(fileUrls, timeSpans, baidu, originalText, language);
        }
